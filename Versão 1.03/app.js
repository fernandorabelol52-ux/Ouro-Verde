...
const RETRY_INICIAL_MS   = 30000;  // retry enquanto a primeira carga não der certo
...
/* ============================================================
   ...
   ============================================================ */
// `keys` são comparadas SEM acento e em minúsculas (ver normalizarCategoria).
// Também aceita variações com sufixo, ex.: "Miúdos (Bovinos)" → miudos, "Suínos (Miúdos)" → suinos.
const CATEGORIAS_CONFIG = [
  { key: 'bovinos',   label: 'BOVINOS',   sub: 'PREÇO POR KG',       cor: '',        keys: ['bovinos','bovino','boi'] },
  { key: 'suinos',    label: 'SUÍNOS',    sub: 'PREÇO POR KG',       cor: '',        keys: ['suinos','suino','porco'] },
  { key: 'aves',      label: 'AVES',      sub: 'PREÇO POR KG',       cor: 'verde',   keys: ['aves','ave','frango','frangos'] },
  { key: 'miudos',    label: 'MIÚDOS',    sub: 'PREÇO POR KG',       cor: 'marrom',  keys: ['miudos','miudo'] },
  { key: 'frios',     label: 'FRIOS',     sub: 'PREÇO POR KG',       cor: 'laranja', keys: ['frios','frio'] },
  { key: 'linguicas', label: 'LINGUIÇAS', sub: 'PREÇO POR KG',       cor: 'roxo',    keys: ['linguicas','linguica'] },
  { key: 'espetos',   label: 'ESPETOS',   sub: 'PREÇO POR UNIDADE',  cor: 'amarelo', keys: ['espetos','espeto'] },
];

/** Normaliza texto de categoria: minúsculas, sem acento, sem espaços extras. */
function normalizarCategoria(cat) {
  return (cat || '').toString().trim().toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '');
}

/** Devolve a config da categoria correspondente ao texto da planilha, ou null. */
function resolverCategoria(cat) {
  const c = normalizarCategoria(cat);
  if (!c) return null;
  return CATEGORIAS_CONFIG.find(cfg =>
    cfg.keys.some(k => c === k || c.startsWith(k + ' ') || c.startsWith(k + '('))
  ) || null;
}

/* ============================================================
   ESCALA AUTOMÁTICA — ajusta o layout 1920×1080 a qualquer tela
   (TV 720p, 4K, navegador de PC com barra de endereço, etc.)
   ============================================================ */
function ajustarEscala() {
  ...
  const tx = Math.round((vw - W * s) / 2);
  const ty = Math.round((vh - H * s) / 2);
...
function construirSlides(dados) {
  ...
  CATEGORIAS_CONFIG.forEach(cfg => {
    ...
    if (!itens.length) return; // categoria sem itens visíveis → não gera slide
    const paginas = paginarItens(itens);

    paginas.forEach((paginaItens, pNum) => {
      const slideId = `slide-${cfg.key}-${pNum}`;

      // Título sempre limpo — slides da mesma categoria não são numerados
...
function goToSlide(targetId) {
  ...
  if (!prevEl || !nextEl) {
    // Estado inconsistente: nunca deixar a rotação parada
...
async function carregarDados() {
  ...
  try {
    ...
    const dados = {};
    CATEGORIAS_CONFIG.forEach(cfg => { dados[cfg.key] = []; });
    const naoReconhecidas = new Set();
    visiveis.forEach(i => {
      const cfg = resolverCategoria(i.categoria);
      if (cfg) dados[cfg.key].push(i);
      else naoReconhecidas.add(i.categoria || '(vazia)');
    });
    if (naoReconhecidas.size) {
      console.warn('[Menu Digital] Categorias não reconhecidas na planilha (itens ignorados):',
        Array.from(naoReconhecidas));
    }
    ...
    if (!carregouUmaVez) {
      // Ainda sem dados (ex.: TV ligou antes do Wi-Fi): mostra aviso
      // e tenta de novo em RETRY_INICIAL_MS em vez de esperar 5 minutos.
      mostrarAvisoConexao();
      clearTimeout(retryTimer);
      retryTimer = setTimeout(carregarDados, RETRY_INICIAL_MS);
    }
  }
}
...
function mostrarAvisoConexao() {
  const wrapper = document.getElementById('slides-wrapper');
  if (!wrapper || wrapper.querySelector('.slide')) return;
  wrapper.innerHTML = `
    <div class="aviso-conexao">
      <img src="logo.png" alt="Açougue Ouro Verde" class="aviso-conexao-logo" />
      <div class="aviso-conexao-txt">Conectando ao cardápio…</div>
      <div class="aviso-conexao-sub">Verifique a conexão com a internet. Tentando novamente automaticamente.</div>
    </div>`;
}
...
console.info(
  `%c🥩 ${NOME_LOJA} – Menu Digital v1.03`,
